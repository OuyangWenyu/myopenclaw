...
#   pin 分散在**九个**文件：3 个 `.env`（`.env` / `.env.zhixun-bot` / `.env.tianyi-bot`）、
#   3 个同名 `.example`（模板是新部署的唯一来源，漏改则新机跑旧版），以及 3 个 compose
#   文件里 `${VAR:-默认值}` 的兜底默认值（主 compose + 两个 bot compose）。
#
#   升级大版本时最容易出的事就是「改了大部分、漏掉其中几个」。而且漏改**不会报错**：
