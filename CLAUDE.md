...
docker compose exec hermes cardamum addressbooks create "Contacts"  # Create addressbook (first use)
docker compose exec hermes cardamum cards list <addrbook-id>        # List all contacts
docker compose exec hermes cardamum cards read <addrbook-id> <id>   # Read contact details
...
- **Contacts (cardamum)**: [cardamum](https://github.com/pimalaya/cardamum) v0.1.0 is installed as a CLI contact manager from the same pimalaya ecosystem as himalaya. Uses **vdir** backend — contacts stored as `.vcf` files in `~/.hermes/.contacts/` (persists on `/opt/data` volume, backed up by backup-cron). Config at `~/.hermes/home/.config/cardamum/config.toml` auto-generated by entrypoint wrapper; symlinked to `/root/.config/cardamum/` for root access. Key commands: `cardamum addressbooks create "Contacts"` (first-time setup), `cardamum cards list <addrbook-id>` (list contacts), `cardamum cards read <addrbook-id> <card-id>` (read details). To add contacts programmatically, write a vCard 4.0 `.vcf` file (with `UID` matching the filename UUID) into the addressbook directory under `/opt/data/.contacts/`. QQ mail and DLUT (Coremail) do not support CardDAV, so the vdir local backend is used instead. Contacts are included in cloud backups via `hermes/scripts/backup.sh`.
