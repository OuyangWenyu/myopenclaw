...
- **Cloud-agnostic backups**: `BACKUP_ROOT` is resolved at runtime from `.cloud.conf` (Google Drive / OneDrive / custom). The host-side scripts (`backup-all.sh`, `restore.sh`) read `.cloud.conf`; the container-side script (`backup-all-docker.sh`) just uses `BACKUP_ROOT=/backup` from the volume mount. **`BACKUP_ROOT` is a HOST env var exported by `start.sh`** — `docker-compose.yml` falls back to `${BACKUP_ROOT:-/tmp/myopenclaw-backups}` when it's absent, and macOS purges `/tmp` after 3 days, so a plain `docker compose up -d backup-cron` silently writes backups somewhere they get deleted. Guard: `start.sh` writes a `.myopenclaw-backup-root` sentinel into the backup root, and `docker/backup-cron/check-cloud-root.sh` makes the container **refuse to start** without it (checks "was this configured", not "is this cloud" — local dirs are equally valid).
