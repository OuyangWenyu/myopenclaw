#!/usr/bin/env python3
"""
Send daily-dev-report summary as a Feishu interactive card to the user's private chat.
...
Usage (inside hermes container):
  echo "## 研发日报" | python3 /opt/hermes-skills/daily-dev-report/tools/send_card.py
...
def main() -> None:
    ...
    today = date.today()
    header = f"研发日报 — {today.month}月{today.day}日 {_weekday_cn(today)}"
    ...
    try:
        ...
        try:
            ...
            pass
        print(
            f"[daily-dev-report] Feishu HTTP {e.code}: {body}",
            file=sys.stderr,
        )
        sys.exit(1)
    except Exception as e:
        print(f"[daily-dev-report] Send failed: {e}", file=sys.stderr)
        sys.exit(1)

    if resp.get("code") == 0:
        print(f"[daily-dev-report] 私聊推送成功 → {header}", file=sys.stderr)
    else:
        print(
            f"[daily-dev-report] 推送失败: code={resp.get('code')} "
            f"msg={resp.get('msg')}",
            file=sys.stderr,
        )
        sys.exit(1)
