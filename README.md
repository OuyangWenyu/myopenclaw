...
**配置邮箱**（可选）：Hermes 支持通过 IMAP/SMTP 收发邮件，配置写在 `~/.hermes/.env` 中。推荐用 Gmail/Outlook，但 Outlook 目前已停用基本认证，Gmail 需要科学上网。以 QQ 邮箱为例：
...
EMAIL_ALLOWED_USERS=允许的邮箱列表，逗号分隔
EMAIL_HOME_ADDRESS=你的QQ号@qq.com
```

> **注意**：① 不要用个人主力邮箱，建一个新邮箱或用小号。② SMTP 端口必须用 **587**（STARTTLS），不能用 465（Hermes 不兼容 SSL 直连）。

重启 Hermes 后查看日志确认连接：`docker compose logs hermes | grep "Email"`

详见 [Hermes 邮箱文档](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/email)。
