...
OpenClaw 容器会读取宿主机 `~/.openclaw/bin/gc`，并把 `/home/node/.openclaw/bin` 放到 `PATH`，所以 Agent 可以直接执行 `gc repo list`、`gc issue create`、`gc pr diff` 等命令。
...
在 OpenClaw 工作区指令 `~/.openclaw/workspace/AGENTS.md` 中添加规则，例如：
